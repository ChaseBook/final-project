...
```{r height_reformat, include=FALSE}

# I reformatted the height string to inches numeric

master <- master %>%
  mutate(ht = sapply(strsplit(master$ht,"-"),
...
```{r redraft_function, include=FALSE}
...
Another important factor to consider when turning down the draft is how a
...
This graph shows the average change in draft rounds for unsigned high school
...
periods together, it is worth noting that the curves are gradually crossing the
y=0 line at an earlier round.

The leftward shift of this intersection means that earlier-round unsigned picks
...
30th to 40th round picks from 2010 to 2015 were redrafted at all.
...
```{r career_peak, echo=FALSE}

peak_level <- function(max_round, startyear, endyear, min_round = 1, beyond_college = "") {
...
top_20_hs <- master %>%
  ...
  mutate(high_level = ifelse(high_level %in% c("-", "JrCollege", "NAIA", "NCAA"),
                             "<=College",
                             high_level)) %>%
  filter(high_level != beyond_college) %>%
...
peak_level(max_round = 20, startyear = 1990, endyear = 2010, min_round = 1, beyond_college = "")
```




```{r debut_age, echo=FALSE}
...
state_mlb %>%
  ...
  labs(
    ...
    subtitle = "From 1965-2012 - Outliers: (California: 2249, Florida: 1069, Texas: 627)",
    color = "MLB Probability"
  )
...
# of the new signing_gap column. Slicing won't affect the calculation but makes
# the data easier to look at.
...
```
The most common age unsigned high school players reach the majors is 24, compared to 22 for signed high school picks. The mean age unsigned high school players reach the MLB is 24.19, compared to 22.65 for signed high school picks. Interestingly, of unsigned high school picks that ultimately reach the majors, the average player waits 2.84 years before signing again. This suggests that of high school picks that reach the majors, ones that go to college spend less time in the minor leagues. The average difference in debut age is approximately 1.54 years, meaning that on average, redrafted high schoolers spend 1.3 fewer years in the minor leagues. Although they still make their debut at a later age than their peers that sign, college baseball may provide players with experience to rise through the minor leagues more efficiently.
...
master %>%
  ...
  slice(1) %>%
  ungroup() %>%
  summarize(debut_age = mean(debut))

master %>%
  filter(source == "H" & signed == "N" & high_level == "MLB") %>%
  group_by(player_id) %>%
  slice(1) %>%
  ungroup() %>%
  summarize(debut_age = mean(debut))
...
```{r}
regression_master <- master %>%
  filter(source == "H" & draft_numb == 1) %>%
  filter(year <= 2012) %>%
  mutate(mlb_made = ifelse(high_level == "MLB", 1, 0)) %>%
  mutate(mlb_made = as.factor(mlb_made))

model_mlb_made <- glm(mlb_made ~ signed, family = binomial, data = regression_master)  %>%
  tidy(conf.int = TRUE)
...
hs_sign_by_round <- function(startyear, endyear = startyear + 4) {
  

by_round <- master %>%
  filter(source == "H") %>%
  filter(year >= startyear & year <= endyear) %>%
  mutate(half_decade = as.factor(ifelse(year %in% 1990:1994, 1990,
                                 ifelse(year %in% 1995:1999, 1995,
                                        ifelse(year %in% 2000:2004, 2000,
                                               ifelse(year %in% 2005:2009, 2005,
                                                      ifelse(year %in% 2010:2014, 2010, 
                                                             ifelse(year %in% 2015:2019, 2015, NA)))))))) %>%
  mutate(signed_numeric = ifelse(signed == "Y", 1, 0)) %>%
  group_by(draft_round, half_decade) %>%
  summarize(percent_signed = sum(signed_numeric) / n(),
            drafted = n(),
            signed = sum(signed_numeric)) %>%
  mutate(percent_signed = ifelse(half_decade == 2010 & draft_round > 40, NA, percent_signed))

by_round %>%
  ggplot(aes(draft_round, percent_signed)) +
  geom_smooth(formula = y~x+x^2, aes(group = half_decade, color = half_decade), se = FALSE, size = 1) +
  theme_light()

}

hs_sign_by_round(startyear = 1990, endyear = 2019)
...
signed_by_year_hs %>%
  ...
  labs(
    x = "Year",
    y = "Percent of Picks Signed",
    title = "Signability of High School Picks Over Time"
  )

signed_by_year_hs %>%
  ggplot(aes(year, percent_signed)) +
  geom_point(aes(color = period)) +
  geom_smooth(formula = y~x+x^2, se = FALSE, aes(color = period)) +
  theme_light() +
  labs(
    x = "Year",
    y = "Percent of Picks Signed",
    title = "Trends in Signability of High School Picks",
    subtitle = "Before and after the draft was shortened to 40 rounds in 2011"
  )
```
...
# making function for signability by round regression discontinuity with
# variable rounds and player source.

draft_signability <- function(min_round = 1, max_round = 100, player_source = c("H", "C")) {

signed_by_year <- master %>%
  filter(draft_round >= min_round & draft_round <= max_round) %>%
  filter(source %in% player_source) %>%
  mutate(signed_numeric = ifelse(signed == "Y", 1, 0)) %>%
  group_by(year) %>%
  summarize(percent_signed = sum(signed_numeric) / n(),
            drafted = n(),
...
master %>%
  ...
  mutate(high_level = ifelse(high_level %in% c("-", "JrCollege", "NAIA", "NCAA"),
                             "<=College",
                             high_level)) %>%
  filter(signed == "Y" & high_level == "<=College" & year <= 2018) %>%
  arrange(desc(year))
```

```{r}
master %>%
  filter(year %in% c(1990:2010) & draft_round %in% c(1:20)) %>%
  filter(source == "H" & draft_numb == 1) %>%
  mutate(high_level = ifelse(high_level %in% c("-", "JrCollege", "NAIA", "NCAA"),
                             "<=College",
                             high_level)) %>%
  filter(signed == "N" & high_level != "<=College" & high_level != "Indy") %>%
  summarize(percent_MLB = sum(high_level == "MLB") / n())
```

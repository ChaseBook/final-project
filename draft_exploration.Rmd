...
mlb_percentile_signed <- function(round = 20, startyear = 1990, endyear = 2010) {
...
top_20_unsigned_total <- master %>%
  filter(source == "H" & signed == "N" & draft_numb == 1) %>%
  filter(draft_round <= 20 & year >= 1990 & year <= 2010) %>%
  count() %>%
  pull(n)
...
mlb_percentile_20u <- top_20_unsigned_mlb / top_20_unsigned_total
